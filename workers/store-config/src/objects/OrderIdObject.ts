export class OrderIdObject {
  ...
  private async sequence() {
    ...
    await this.state.storage.transaction(async (txn) => {
      ...
      val = val === undefined || val === OrderIdObject.sequenceLength ? 0 : val;
  ...
  async fetch(request: Request) {
    ...
    const nextSeqPadded = nextSeq.toString().padStart(Math.log10(OrderIdObject.sequenceLength), '0');

    // Make value.
    const result = new Date().toISOString() + nextSeqPadded;

    // Obfusciate with hash, chance of collision is 16^hashLength.
    const hashedResult = HT_PREFIX + (await this.hash(result)).slice(0, OrderIdObject.hashLength);

    // Return response.
    console.debug(`Generated Order ID: ${hashedResult}, DO value: ${nextSeq}.`);
