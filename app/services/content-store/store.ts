...
import { splitCacheKey } from './utils';
...
async function getContentByTypeSlug(
    ...
    if (context.CONTENT_STORE_CACHE_ENABLED) {
        ...
        // Fetch data from Notion TODO revisit, this is not needed
...
// Get all content urls for a given type
async function listAllContentByType(
    context: HTAppLoadContext,
    type: ContentType
): Promise<ContentStoreEntry[]> {
    const data: ContentStoreEntry[] = [];
    const entries = await listAllKeysByType(context, type);
    if (entries) {
...
async function listAllContent(
    ...
    for (const type of ['blog', 'faq', 'product']) {
        data.push(
            ...(await listAllContentByType(context, type as ContentType))
        );
    }
    return data;
}
...
function makeMetadata(entry: any) {
    const title =
        entry.Question?.title[0].text.content ??
        entry.Post?.title[0].text.content ??
        entry.Title.rich_text[0].text.content ??
        undefined;
    const tags = entry['Tags']?.multi_select.map((x) => x.name);
    return {
        title,
        tags,
    };
...
async function purgeAndRebuildByType(
    ...
    for (const entry of data) {
        try {
            ...
            const metadata = makeMetadata(entry);
            await putEntry(
                context,
                urlPath.type,
                urlPath.slug,
...
export {
    validateRequest,
    getContentByTypeSlug,
    listAllContentByType,
    listAllContent,
    purgeAndRebuildByType,
    purgeAndRebuildAll,
};
